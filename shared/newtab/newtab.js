...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // shared/js/newtab/ddg-feed-header.js
  var DDGFeedHeader = class extends s4 {
    render() {
      return y`
            <div class="root">
                <span class="shield">
  ...
  __publicField(DDGFeedHeader, "styles", [
    i`
          .root {
            padding-left: 44px;
            position: relative;
  ...
  __publicField(DDGFeedHeader, "properties", {
...
`;
  ...
  var TopSites = class extends s4 {
    ...
    get toggleUI() {
      const text = this.state === "idle" ? "Show Most Visited Sites" : "Hide Most Visited Sites";
      return y`
            <div class="control">
    ...
    get list() {
      if (this.topSites.length === 0) {
        return y`nothing to see`;
      }
      return y`
            <ul class="list">
                ${c4(this.topSites.slice(0, 6), (x2) => x2.title, (x2) => {
        return y`
                        <li class="item">
                            <ddg-top-site .url=${x2.url} .favicon=${x2.favicon} .title=${x2.title}></ddg-top-site>
                        </li>
                    `;
      })}
            </ul>
    ...
    render() {
      return y`
            <div class="root">
                ${this.state === "permission-granted" ? [this.list, this.toggleUI] : this.toggleUI}
            </div>
        `;
    }
  };
  ...
  __publicField(Search, "properties", {});
  ...
  var Layout = class extends s4 {
    firstUpdated(_changedProperties) {
      super.firstUpdated(_changedProperties);
      fetch("https://duckduckgo.com/ac/?q=nike&kl=wt-wt").then((x2) => {
        return x2.json();
      }).then((x2) => {
        console.log(x2);
      });
